<template>
  <div :class="themeClass" class="min-h-screen flex items-center justify-center px-6 transition-colors duration-300">
    <!-- Navbar -->
    <nav :class="['w-full py-4 px-8 flex items-center justify-between fixed top-0 left-0 right-0 z-50 shadow', themeClass]" 
        ...
        <form @submit.prevent="handleNextStep" class="space-y-6">
          ...
          <div v-if="step === 1" class="space-y-4">
            <div v-for="field in firstPartFields" :key="field.id" class="space-y-2">
              <label class="block text-lg font-medium" :for="field.id">{{ field.label }}</label>
              <input :type="field.type" :id="field.id" v-model="firstPartData[field.model]" required class="w-full px-4 py-2 rounded-lg border focus:outline-none focus:ring-2 transition" :class="inputClass" />
            </div>
          </div>
          ...
          <div class="flex justify-between">
            <button v-if="step === 2" @click="step = 1" type="button" class="px-6 py-3 rounded-lg shadow transition text-lg font-semibold" :class="buttonClass">
              Back
            </button>
            <button type="submit" class="px-6 py-3 rounded-lg shadow transition text-lg font-semibold" :class="buttonClass" @click="signUp">
  ...
  data() {
    return {
      ...
      firstPartFields: [
        ...
        { id: "confirmPassword", label: "Confirm Password", type: "password", model: "confirmPassword" },
      ],
      secondPartFields: [
        { id: "about", label: "About", type: "text", model: "about", tag: "textarea" },
        { id: "github", label: "GitHub URL", type: "url", model: "github", tag: "input" },
        { id: "linkedin", label: "LinkedIn URL", type: "url", model: "linkedin", tag: "input" },
        { id: "resume", label: "Upload Resume", type: "file", model: "resume", tag: "input" }
      ],
    };
  },
  computed: {
