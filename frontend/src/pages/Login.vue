<template>
  <div :class="themeClass" class="min-h-screen flex flex-col transition-colors duration-300">
    <Navbar :theme="theme" :isLoggedIn="isLoggedIn" @toggle-theme="toggleTheme" :isLogin=true />
...
const inputClass = computed(() => theme.value === 'light' ? 'border-gray-300 focus:ring-light-accent' : 'border-gray-600 focus:ring-dark-accent');
...
const handleLogin = () => {
  ...
  isLoggedIn.value = true; // Update session state
  router.push('/profile');
};
