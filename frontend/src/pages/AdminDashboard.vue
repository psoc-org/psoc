		...
		<div class="pt-20 px-6 w-full">
			...
			<div class="grid grid-cols-1 md:grid-cols-3 mt-10 gap-8 w-full">
				<LineChart
					...
					title="Contributor Proposals Submitted vs Time"
					:theme="theme"
				/>
				<LineChart
					class="w-full"
					:data="projectsData"
					title="Projects Submitted by Organization vs Time"
					:theme="theme"
				/>
				<LineChart
					class="w-full"
					:data="organizationsData"
					title="Organizations Registered vs Time"
					:theme="theme"
				/>
			</div>
			...
			<div class="mt-14">
				<h2 :class="textClass" class="text-3xl font-extrabold mb-8 text-center">
					Organization Approvals
				</h2>
					...
					>
						...
						<CardFooter class="flex gap-4 mt-6 justify-center">
							...
							<Button
								...
								class="bg-green-800 px-6 py-3 text-lg rounded"
								@click="approveOrganization(org.id)"
								>Approve</Button
							>
							<Button
								:class="buttonClass"
								class="bg-red-800 px-6 py-3 text-lg text-white rounded"
								@click="declineOrganization(org.id)"
								>Decline</Button
							>
						</CardFooter>
					</Card>
...
const organizations = ref([
	{
		...
		founded: '2001',
		location: 'USA',
		description: 'Manages the Python language.',
		approved: false,
		declined: false,
	},
	{
	...
	},
	{
		...
		founded: '2007',
		location: 'USA',
		description: 'Supports open-source development.',
		approved: false,
		declined: true,
	},
])
...
const stats = computed(() => {
	return {
		pending: {
			label: 'Pending',
			value: organizations.value.filter((o) => !o.approved && !o.declined).length,
		},
		approved: {
			label: 'Approved',
			value: organizations.value.filter((o) => o.approved).length,
		},
		declined: {
			label: 'Declined',
			value: organizations.value.filter((o) => o.declined).length,
		},
...
const textClass = computed(() => (theme.value === 'dark' ? 'text-dark-text' : 'text-light-text'))
